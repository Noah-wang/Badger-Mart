...
function calculateSubtotal() {
	...
	return totalPrice; // TODO calculateSubtotal
}

function calculateSalesTax() {
	const subtotal = calculateSubtotal();
	var state = document.getElementById("state-tax").value;
	const taxRate = getSalesTaxRateForState(state);	
	const saleTax = subtotal*taxRate;
	const rounded = roundMoney(saleTax);
	return rounded; // TODO calculateSalesTax
}
...
function calculateSubtotalWithTax(){
	var totalPrice = 0;	
	for(var i = 0; i< ITEMS.length;i++){
		const quantity = parseInt(document.getElementById(ITEMS[i]+"-quantity").value);
		const price = parseFloat(document.getElementById(ITEMS[i]+"-price").innerText);
...
document.getElementById("btn-checkout").addEventListener("click", () =>{
	alert("Your total is: $" + calculateSubtotalWithTax());
});
